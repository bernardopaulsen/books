...
class Snake():
    ...
    def __init__(self,
        ...
        self.positions = self.start_positions(size)

    @staticmethod
    def start_positions(size : int) -> list:
        """
        Returns start positions for the snake (bottom left)
        ...
        -------
        positions : list
            Start positions for the snake.
        """
        return [[i,0] for i in range(size)]

    def check_crash(self, directions: list, x_size: int, y_size: int) -> bool:
        """
        Returns True if snake eats itself or gets out of the table.

        Params
        ------
        directions : list
            Directions for where the snake shall move.
        x_size : int
            Length of table.
        y_size : int
            Height of table.
        ...
        -------
        crash : bool
            True if snake eats itself or gets out of the table. 
        """
        crash     = False
...
class Table():
    ...
    def print(self, time : float, i : int):
        #os.system('clear')
...
class Game():
    ...
    def simulate(self):
        hp.setrelheap()
        self.table.print(time.time()-self.start,0)
        print("heapy after move 0")
        h = hp.heap()
        print(h)
        hp.setrelheap()
        for i in range(self.max_steps):
            ...
            self.table.print(time.time()-self.start,i+1)
            if self.table.check_eaten():
                self.table.change_food()
                self.table.snake.grow()
            print(f"heapy after move {i+1}")
            h = hp.heap()
            print(h)
